...
class MainActivity : AppCompatActivity() {
    ...
    private fun initViews() {
        ...
        btnQuickDnsTest.setOnClickListener { runQuickDnsTest() }
    ...
    private var quickTestThread: Thread? = null

    private fun runQuickDnsTest() {
        if (quickTestThread?.isAlive == true) {
            quickTestThread?.interrupt()
            quickTestThread = null
            btnQuickDnsTest.text = getString(R.string.quick_dns_test_button)
            btnQuickDnsTest.backgroundTintList = android.content.res.ColorStateList.valueOf(Color.parseColor("#0D47A1"))
            return
        }
        ...
        btnQuickDnsTest.text = "\u23F9 Stop"
        btnQuickDnsTest.backgroundTintList = android.content.res.ColorStateList.valueOf(Color.parseColor("#B71C1C"))
        tvReportContent.setTextColor(Color.parseColor("#AAAAAA"))
        tvReportContent.text = getString(R.string.quick_dns_test_running)

        quickTestThread = Thread {
            ...
            try {
                ...
                if (leakComparison.ispResult.error != null) {
                    ...
                    for (r in leakComparison.ispResult.resolverIps) {
                        display.appendLine("    \u2022 ${r.ip}${r.country?.let { " \u2014 $it" } ?: ""}${r.isp?.let { " ($it)" } ?: ""}")
                    }
                }
            ...
            runOnUiThread { tvReportContent.text = display.toString() + "\u23F3 ${getString(R.string.report_testing_blocking)}" }

            // 2. URL Blocking test (multi-domain)
            ...
            for (domain in testDomains) {
                ...
                runOnUiThread { tvReportContent.text = display.toString() + "\n\u23F3 ${getString(R.string.report_testing_blocking)}" }
            }
